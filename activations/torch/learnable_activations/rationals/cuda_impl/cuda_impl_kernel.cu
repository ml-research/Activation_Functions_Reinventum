...
template <typename scalar_t>
__global__ void rationals_cuda_forward_kernel(
...
template <typename scalar_t>
__global__ void rationals_cuda_backward_kernel(
...
void rationals_cuda_forward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(x.scalar_type(), "rationals_cuda_forward", ([&] {
        rationals_cuda_forward_kernel<scalar_t><<<blocks, threads>>>(
...
void rationals_cuda_backward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(x.scalar_type(), "rationals_cuda_backward", ([&] {
        using accscalar_t = at::acc_type<scalar_t, true>;
        size_t shared_mem_size = (M + N) * threads * sizeof(accscalar_t);

        rationals_cuda_backward_kernel<scalar_t><<<blocks, threads, shared_mem_size>>>(
