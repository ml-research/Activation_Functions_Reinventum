...
class RegisteredModule:
    ...
    def save_inputs(
        self, name, saving=True, max_saves=-1, bin_width=None, mode=None, label=None
    ):
        ...
        self._input_handle = self.module.register_forward_hook(
            _input_hook(
                self,
                self.input_distributions[name][0],
                max_saves,
                idx=idx,
            )
        )
    ...
    def save_gradients(
        ...
        self.input_gradient_distributions[name] = (
            histogram.Histogram(bin_width),
            label_in,
        )
        self.output_gradient_distributions[name] = (
            histogram.Histogram(bin_width),
            label_out,
        )
    ...
    def plot_histogram(
        self, hist, label, axis, tolerance=0.001, use_kde=False, color=None
    ):
    ...
    def _plot_histogram(
        self, weights, bins, bin_size, axis, kde_fn=None, label=None, color=None
    ):
        if kde_fn is None:  # display mode 'bar'
            ...
            if len(bins) < 5:
                msg = (
                    f"Too few bins, maybe reduce bin size. Expected at least 5, got {len(bins)}"
                )
            ...
            y = kde_fn(x)
            axis.fill_between(x, y, alpha=0.45, label=label, color=color)
        ...
        if name is not None:
            ...
            if other_func is not None:
                for other_func_name in other_func:
                    axis.plot(
                        x, other_func(x), label=other_func_name, color=next_color()
                    )
    ...
    def show_inputs(
        self, axis, color=None, name="snapshot_0", tolerance=0.001, use_kde=False
    ):
...
class ActivationModule:
    ...
    _count = 0
    _step = 0
    _logger = ActivationLogger("ActivationModule")
    _plotting_style = {}
    _default_irm = "layer"
    _color_cycle = None
    ...
    def get_groups(cls, group):
        ...
        names = tuple(
            filter(
                lambda name: not set(cls._registered_modules[name].groups).isdisjoint(
                    _groups
                ),
    ...
    def get_snapshots(cls, name=None, group=None):
        ...
        snapshots = [
            snapshot
            for snapshot in cls._snapshot_names
        ...
        if returns:
            ...
            for module in modules:
                captures[module.name] = module.capture(
                    name=snap_name, other_func=other_func, returns=True
                )
            return captures
    ...
    def show_function(
        ...
        function=False,
        inputs=False,
        gradients_input=False,
        gradients_output=False,
        x_mode="expand",
        tol_in=0.001,
        tol_grad_in=0.001,
        tol_grad_out=0.001,
        ...
        if axes is not None:
            ...
            with plt.rc_context(rc=cls._plotting_style):
                fig, axes = plt.subplots(
                    *layout, figsize=figsize, squeeze=True, **fig_kw
                )
