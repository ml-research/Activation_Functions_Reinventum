...
import rationals_cuda_inline  # Ensure this module is correctly implemented and available
from activations.torch import activation_module
...
RationalCudaInline = RationalsCUDAActivation()
...
def run_profiling():
    ...
    # Activations to be compared
    activations = [RationalCudaInline, Rational, ReLU]

    for activation in activations:
        # Determine if the activation has parameters
        has_params = any(p.requires_grad for p in activation.parameters())

        # Select applicable configurations based on whether activation has parameters
        if has_params:
            applicable_configs = base_configurations
        else:
            # Only use configurations without additional activation optimizer
            applicable_configs = [c for c in base_configurations if not c['additional_activation_optimizer']]
        ...
        for config in applicable_configs:
            ...
            unique_name = f"{config_name}_{activation_name}_{timestamp}"
            profiler_log_file = os.path.join(profiler_log_dir, f"{unique_name}_profiler.txt")

            try:
                # Initialize the model with the desired configuration and activation
                model = SimpleViT(
                    image_size=IMG_SIZE,
                    patch_size=16,
                ...
                # Initialize the Logger with a unique name per configuration and activation
                ...
                trainer = pl.Trainer(
                    ...
                    precision=config['precision'],  # '16-mixed' or 32
                    log_every_n_steps=1,
                    logger=logger,
                ...
                with open(profiler_log_file, 'w') as f:
                    f.write(f"Configuration: {config['optimizer_setup']} with Precision {config['precision']}-bit and Activation {activation_name}\n")
                ...
                print(f"Completed: Activation={activation_name}, Optimizer Setup={config['optimizer_setup']}, Precision={config['precision']}-bit, Val Loss={final_val_loss:.4f}, Val Acc={final_val_acc:.4f}, Training Time={training_time:.2f}s")
                ...
                logging.error(f"An error occurred during configuration '{unique_name}': {e}")
    ...
    def print_summary(results):
        print("\n===== Profiling Results =====")
        for res in results:
            activation = res.get('Activation', 'N/A')
            optimizer = res.get('Optimizer Setup', 'N/A')
            precision = res.get('Precision', 'N/A')
            val_loss = res.get('Validation Loss', 'N/A')
            val_acc = res.get('Validation Accuracy', 'N/A')
            training_time = res.get('Training Time (s)', 'N/A')
            inference_time = res.get('Average Inference Time (ms)', 'N/A')
            print(f"Activation: {activation}, Optimizer: {optimizer}, Precision: {precision}, "
                  f"Val Loss: {val_loss}, Val Acc: {val_acc}, "
                  f"Training Time: {training_time:.2f}s, "
                  f"Avg Inference Time: {inference_time:.2f} ms")

    # =============================
    # Save Results Function
    # =============================
    def save_results(results, output_file='profiling_results.json'):
        """
        ...
        try:
            ...
            logging.info(f"Saved profiling results to {output_file}")
        except Exception as e:
            logging.error(f"Failed to save profiling results to {output_file}: {e}")
    ...
    def test_with_imagenette():
        ...
        try:
            ...
            logging.error(f"Failed to load profiling_results.json: {e}")

        # Save the results to a JSON file
        save_results(results, 'profiling_results.json')
